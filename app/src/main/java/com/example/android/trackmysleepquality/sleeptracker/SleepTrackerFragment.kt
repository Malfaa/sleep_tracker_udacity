...
class SleepTrackerFragment : Fragment() {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?,
        ...
        val adapter = SleepNightAdapter(SleepNightListener { nightId -> sleepTrackerViewModel.onSleepNightCicked(nightId)})
        ...
        //val managerLinear = LinearLayoutManager(activity)
        ...
        sleepTrackerViewModel.navigateToSleepQualityData.observe(viewLifecycleOwner, Observer {
            night ->
            night?.let {
                this.findNavController().navigate(SleepTrackerFragmentDirections.actionSleepTrackerFragmentToSleepDetailFragment(night))
                sleepTrackerViewModel.onSleepNightQualityNavigated()
            }
        })
        return binding.root
    }
